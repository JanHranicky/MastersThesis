...
class CA(tf.keras.Model):
  ...
  def set_rule_model(self,channel_n):
    return tf.keras.Sequential([
      tf.keras.layers.Conv2D(filters=512,kernel_size=1,activation='elu'),
      tf.keras.layers.BatchNormalization(),
      tf.keras.layers.Conv2D(filters=256,kernel_size=1,activation='elu'),
      tf.keras.layers.BatchNormalization(),
      ...
      tf.keras.layers.BatchNormalization(),
      tf.keras.layers.Conv2D(filters=16,kernel_size=1,activation='elu'),
      tf.keras.layers.BatchNormalization(),
      tf.keras.layers.Conv2D(filters=8,kernel_size=1,activation='elu'),
      tf.keras.layers.BatchNormalization(),
      tf.keras.layers.Conv2D(filters=4,kernel_size=1,activation='elu'),
      tf.keras.layers.BatchNormalization(),
...
class discreteOutTrainer(trainer.Trainer):
    ...
    def generate_gif(self,i,w,h):
      ...
      imgs = run_model_for_i(self.model,w,h,self.model.channel_n,self.train_step_interval[1],grayscale=self.grayscale)
...
ca = CA(channel_n=3,model_name=date_time+'_'+os.path.basename(__file__).split('.')[0]+"_more_fully_connected_layers_3_channels")
