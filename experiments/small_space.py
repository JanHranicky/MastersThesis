from core import utils,bigger_model,data_pool
...
class Trainer():
  def __init__(self,model,loss_f,gt_img,gt_img_name,grayscale=False,data_pool_training=False,lr=0.001,epoch_num=30000,visualize=True,visualize_iters=100,save_iters=5000,generate_gif_iters=5000,train_step_interval=(75,100)) -> None:
    ...
    self.loss_f = loss_f
    self.gt_img = gt_img
    self.gt_tf = self.convert_to_tf(gt_img)
  ...
  def train_step(self,x,trainer):
    ...
    with tf.GradientTape() as g:
      for i in tf.range(iter_n):
        x = self.model(x)
        l_x = utils.convert_to_comparable_shape(x,1)
      loss = self.loss_f(self.gt_tf, l_x)
  ...
  def train(self):
    ...
    for i in range(self.epoch_num):
      if self.data_pool_training:
        ...
        converted = utils.convert_to_comparable_shape(x0,1)
        highest_loss_i = self.dp.get_highest_loss_index(self.gt_tf,converted,self.loss_f)
      ...
      #tf.print(utils.convert_to_comparable_shape(x,1)[0],summarize=-1)
  ...
  def convert_to_tf(self,img):
    img = utils.pil2tf(img)[:,:,0]
    white_indices = self.get_value_indices(img,255.)
    black_indices = self.get_value_indices(img,0.)
    img = tf.tensor_scatter_nd_update(img,white_indices,tf.zeros(shape=(white_indices.shape[0],)))
    img = tf.tensor_scatter_nd_update(img,black_indices,tf.ones(shape=(black_indices.shape[0],)))
    
    return img
    
  def get_value_indices(self,t,val):
    mask = tf.equal(t, val * tf.ones_like(t))
    return tf.where(mask)


black = [0,0,0]
white = [255,255,255]
random_colors = [[random.randint(0, 255) for _ in range(3)] for _ in range(8)]
colors = [black, white] + random_colors

GT_IMG_PATH = './img/xhrani02.png'
date_time = datetime.now().strftime("%m_%d_%Y")
gt_img = Image.open(GT_IMG_PATH)

ca = bigger_model.CA(channel_n=1,model_name=date_time+'_'+os.path.basename(__file__).split('.')[0])
