...
import random
from masks import xhrani02_100x100_mask
...
def custom_mse(x, gt, states):
    ...
    loss_tf = tf.square(l_x - gt)
    
    logo_loss = (loss_tf * xhrani02_100x100_mask.logo_mask)*100
    bcknd_loss = loss_tf * xhrani02_100x100_mask.bcknd_mask
    
    return tf.reduce_mean(logo_loss + bcknd_loss)

ca = added_conv_model.CA(channel_n=CHANNEL_NUM,model_name=date_time+'_'+os.path.basename(__file__).split('.')[0]+"_"+str(STATES)+"_states_single_c_compare_"+str(CHANNEL_NUM)+"_channels")
