...
class discreteOutTrainer(trainer.Trainer):
    ...
    def train_step(self,x,trainer):
      ...
      with tf.GradientTape() as g:
        ...
        l_x = utils.convert_to_comparable_shape(x,self.compare_channels)
        #l_x = tf.math.round(l_x)
...
ca = added_conv_model.CA(channel_n=CHANNEL_NUM,model_name=date_time+'_'+os.path.basename(__file__).split('.')[0]+"_batch_"+str(STATES)+"_states_single_compare_"+str(CHANNEL_NUM)+"_channels",rule_model="batch")
#ca.load_weights("./checkpoints/01_08_2024_output_in_range_batch_8_states_single_C_xhrani02_100x100/500")
