...
def train_step(x):
  ...
  with tf.GradientTape() as g:
    for i in tf.range(iter_n):
      x = ca(x)
    print(loss_f(gt_img, utils.to_rgba(x)))
    loss = tf.math.reduce_mean(loss_f(gt_img, utils.to_rgba(x)))
...
for i in range(101):
  ...
  if i%GENERATE_GIF_ITERS == 0:
    imgs = utils.run_model_for_i(ca,width,height,CHANNELS,TRAIN_STEP_INTERVAL[1])
